...
// Middleware
// =======================
app.use(cors({ origin: true })); // allow all (safe for testing, tighten later)
...
app.post("/api/upload-pdf", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!pdfHash) {
      try {
        pdfHash = await computeFileSha256Hex(
          path.join(STORAGE_DIR, filename)
        );
      } catch (e) {
        console.warn("Failed compute hash", e);
      }
    }
...
app.post("/api/sign-pdf", async (req, res) => {
  try {
    ...
    if (!origExists)
      return res
        .status(400)
        .json({ error: `original PDF not found: ${origPath}` });

    const bytes = await fs.readFile(origPath);
    const beforeHash = sha256Hex(bytes);

    const pdfDoc = await PDFDocument.load(bytes);
    const imgData = signatureBase64.includes(",")
    ...
    const docRecord = {
      ...
      afterHash,
      signatureMeta: {
        mime: signatureBase64.startsWith("data:image/png")
          ? "image/png"
          : "image/jpeg",
      },
    };
    ...
    if (DocumentModel) {
      DocumentModel.create(docRecord)
        .then((s) =>
          console.log("Saved audit record to MongoDB", s._id)
        )
        .catch((e) =>
          console.error("DB save failed (async):", e.message || e)
        );
    }

    return res.json({
      success: true,
      url: `/storage/${path.basename(outPath)}`,
      beforeHash,
      afterHash,
    });
  } catch (err) {
